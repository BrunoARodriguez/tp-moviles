...
//validamos que no sean nulos los campos
        if (titulo.isEmpty() || descripcion.isEmpty() || (caloria <= 0)) {
            ...
            if (titulo.length() >= 5 && descripcion.length() >= 10) {
                Toast.makeText(AltaPlatoActivity.this, "Su plato a sido dado de alta! ", Toast.LENGTH_LONG);
                this.plato = new Plato(1, titulo, descripcion, precio, caloria);

            }
            else
            Toast.makeText(AltaPlatoActivity.this,"El titulo debe ser mayor a 4 caracteres y la descripcion mayor a 9",Toast.LENGTH_LONG);
